...
describe('FileViewer manual edit regressions', () => {
  ...
  it('posts the selected target marker to the iframe when opening the inspector from selection', async () => {
    const source = '<!doctype html><html><body><a data-od-id="cta" href="/start">Start</a><p data-od-id="body">Body</p></body></html>';
    vi.stubGlobal('fetch', vi.fn(async () =>
      new Response(source, { status: 200, headers: { 'Content-Type': 'text/html' } }),
    ));
    ...
    act(() => {
      window.dispatchEvent(new MessageEvent('message', {
        data: { type: 'od-edit-select', target: linkTarget() },
        source: frame.contentWindow,
      }));
    });
    await waitFor(() => {
    ...
    // Opening from selection pins the inspector for the selected target...
